...
describe "launch mode" do
  ...
  it "supports @ as the command action menu entrypoint" do
    with_launch_temp_root do
      stdout = IO::Memory.new
      stderr = IO::Memory.new
      code = Actra::CLI.run(["@", "--mode", "background", "--dry-run", "printf", "ok"], IO::Memory.new, stdout, stderr)
  ...
  it "passes a command path and command line to the assistant mode" do
    stdout = IO::Memory.new
    stderr = IO::Memory.new
    code = Actra::CLI.run(["@", "--mode", "assistant", "--dry-run", "printf", "ok"], IO::Memory.new, stdout, stderr)
  ...
  it "can dry-run a local command without backgrounding it" do
    stdout = IO::Memory.new
    stderr = IO::Memory.new
    code = Actra::CLI.run(["@", "--mode", "run", "--dry-run", "printf", "ok"], IO::Memory.new, stdout, stderr)
