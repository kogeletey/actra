...
describe Actra::Activation do
  it "prints zsh dispatch hooks for @ commands" do
    ...
    script.should contain("\\?*) _actra_query")
    script.should_not contain("@stats|@statistics)")
    ...
    script.should contain("ACTRA_AT_MODES=(actions files)")
    script.should contain("files) echo \"context\"")
    script.should_not contain("agents) echo \"agents\"")
    script.should contain("agents\" \"*) query=\"${query#agents }\"")
    script.should contain("context\" \"*) query=\"${query#context }\"")
    ...
    script.should contain("agent|run|background|remote|container)")
  ...
  it "prints bash dispatch hooks for @ commands" do
    ...
    script.should contain("ACTRA_AT_MODES=(actions files)")
    script.should contain("files) printf '%s' \"context\"")
