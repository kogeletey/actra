...
module Actra
  ...
  class ToolRegistry
    ...
    def initialize(@tools : Hash(String, ToolSpec))
    end

    def self.default(include_builtin : Bool, extension_tools : Array(ToolSpec), allowlist : Array(String)? = nil) : ToolRegistry
    ...
    private def execute_external(tool : ToolSpec, args : JSON::Any) : String
      ...
      status = Process.run("/bin/sh", ["-c", command], input: IO::Memory.new(input), output: output, error: error)
      text = output.to_s
      err = error.to_s
      begin
        any = JSON.parse(text)
    ...
    private def run_shell(command : String) : String
      output = IO::Memory.new
      error = IO::Memory.new
      status = Process.run("/bin/sh", ["-c", command], output: output, error: error)
      parts = [] of String
