...
module Actra
  class CLI
    def self.run(argv : Array(String), stdin : IO = STDIN, stdout : IO = STDOUT, stderr : IO = STDERR) : Int32
      ...
      return run_at_help(stdout) if argv[0] == "@?"
      return run_at_file(stdin, stdout, stderr) if argv[0] == "@file"
    ...
    private def self.agent_invocation?(argv : Array(String)) : Bool
      first = argv[0]
      return true if {"-p", "--print", "--mode", "--provider", "--model", "--api-key", "--thinking", "--models", "--list-models", "--system-prompt", "--append-system-prompt", "--no-context-files", "-nc", "--no-session", "--session", "--session-dir", "-c", "--continue", "-r", "--resume", "--fork", "--export", "-e", "--extension", "--no-extensions", "--skill", "--no-skills", "--prompt-template", "--no-prompt-templates", "--tools", "-t", "--no-tools", "-nt", "--no-builtin-tools", "-nbt"}.includes?(first)
      return true if !known_command?(first) && !looks_like_tool_ref?(first)
      false
    end

    private def self.known_command?(first : String) : Bool
      {"shell", "oas", "help", "ain", "auth", "activate", "config", "dispatch", "query", "launch", "complete", "completion", "agent", "@", "@?", "@file", "package", "pkg", "install", "remove", "uninstall", "update", "list", "render", "__cr-extension"}.includes?(first)
    end
    ...
    private def self.run_at_file(stdin : IO, stdout : IO, stderr : IO) : Int32
      unless Interactive::PickerFzf.available?
        stderr.puts "@file requires fzf"
        return 1
      end

      path = Interactive::PickerFzf.pick_file("Files")
      unless path
        stderr.puts "no file selected"
        return 1
      end

      action = Interactive::PickerFzf.pick_one("File action", ["attach @file", "open in editor", "copy absolute path"])
      case action
      when "attach @file"
        stdout.puts Interactive::PickerFzf.context_tokens([path])
        0
      when "open in editor"
    ...
    private def self.run_at_help(stdout : IO) : Int32
      stdout.puts <<-TXT
      Actra @ shortcuts

        @file              Pick a file, then attach, open, or copy its absolute path.
        @path/to/file      Attach a file to an agent prompt.
        @code, @plan       Send a task to a configured actor.
        @actor@domain      Send a ForgeFed task to a remote actor inbox.
        @?                 Show this help.
        @ <command...>     Choose how to run a command: background, assistant, or local.

      Use shell completion after `actra activate <bash|zsh>` to complete @ files and actor commands.
      TXT
      0
    end
    ...
    private def self.run_complete(argv : Array(String), stdout : IO, stderr : IO) : Int32
      ...
      candidates =
        ...
        when "files", "file"
          Interactive::PickerFzf.file_context_candidates(prefix)
        else
    ...
    private def self.at_completion_candidates(prefix : String) : Array(String)
      candidates = actor_completion_candidates + Interactive::PickerFzf.file_context_candidates(prefix)
      candidates.select { |candidate| prefix.empty? || candidate.starts_with?(prefix) }.uniq.sort
    end

    private def self.actor_completion_candidates : Array(String)
      cfg = Config.load
      commands = ["@", "@?", "@file", "@claude", "@codex"] of String
    ...
    private def self.usage(io : IO) : Int32
      ...
      Commands:
        ...
        actra agent [-p] [--mode text|json|rpc] [--provider NAME] [--model MODEL] [@files...] [prompt...]
        actra -p [--provider NAME] [--model MODEL] [@files...] [prompt...]
        actra @ <command...>     # choose background, assistant, or local run
        actra @file              # pick a file, then attach/open/copy its path
        actra @?                 # show @ shortcut help
        actra complete <at|actors|files> [prefix]
        ...
        actra render [--render MODE] [--in FILE]
