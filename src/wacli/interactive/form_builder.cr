...
module Wacli::Interactive
  module FormBuilder
    def self.fields_for_schema(schema : OpenAPI::Dto::Schema, pointer_prefix : String = "") : Array(Wacli::Render::FieldRule)
    ...
    private def self.fields_for_node(schema : OpenAPI::Dto::Schema, pointer : String, name : String, required : Bool) : Array(Wacli::Render::FieldRule)
    ...
    private def self.kind_for(schema : OpenAPI::Dto::Schema) : Wacli::Render::FieldKind
      if schema.enum_values.any?
        return Wacli::Render::FieldKind::Enum
      end
      ...
      if t == "boolean"
        return Wacli::Render::FieldKind::Boolean
      end

      if t == "string" && fmt == "date-time"
        return Wacli::Render::FieldKind::DateTime
      end

      if t == "string" && fmt == "binary"
        return Wacli::Render::FieldKind::File
      end
      ...
      if t == "integer" || t == "number" || schema.arrayish? || schema.ref || t.nil?
        return Wacli::Render::FieldKind::Json
      end

      Wacli::Render::FieldKind::String
    end
