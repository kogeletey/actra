...
tmp="$(mktemp -d)"
formula_out=".meta/packaging/homebrew/actra.rb"
formula_backup="$tmp/actra.rb.backup"
...
exact_out="$(bash "$resolver" "1.2.3" "kogeletey/actra" "v1.2.3" "$fixtures/homebrew-sha256sums-exact.txt")"
assert_eq \
  "$(kv_value BREW_ASSET "$exact_out")" \
  "actra-1.2.3-darwin-arm64-dynamic.tar.gz" \
...
assert_eq \
  ...
  "https://github.com/kogeletey/actra/releases/download/v1.2.3/actra-1.2.3-darwin-arm64-dynamic.tar.gz" \
  "exact resolver produced wrong URL"

fallback_stderr="$tmp/fallback.stderr"
fallback_out="$(bash "$resolver" "1.2.3" "kogeletey/actra" "v1.2.3" "$fixtures/homebrew-sha256sums-fallback.txt" 2>"$fallback_stderr")"
assert_eq \
  "$(kv_value BREW_ASSET "$fallback_out")" \
  "actra-1.2.3-darwin-amd64-dynamic.tar.gz" \
...
set +e
bash "$resolver" "1.2.3" "kogeletey/actra" "v1.2.3" "$fixtures/homebrew-sha256sums-no-darwin.txt" >"$tmp/no-darwin.out" 2>"$tmp/no-darwin.err"
no_darwin_status=$?
set -e
...
set +e
bash "$resolver" "1.2.3" "kogeletey/actra" "v1.2.3" "$tmp/missing-sha.txt" >"$tmp/missing.out" 2>"$tmp/missing.err"
missing_status=$?
set -e
...
bash "$generator" \
  "1.2.3" \
  "https://github.com/kogeletey/actra/releases/download/v1.2.3/actra-1.2.3-darwin-amd64-dynamic.tar.gz" \
  "cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc" \
  "https://github.com/kogeletey/actra"

grep -q 'homepage "https://github.com/kogeletey/actra"' "$formula_out" || fail "formula homepage was not templated"
